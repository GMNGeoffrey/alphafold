# We might also be able to just `pip install jax[rocm]`, but I didn't see that option till after I'd already done this
FROM rocm/jax:rocm7.0-jax0.6.0-py3.12

SHELL ["/bin/bash", "-euo", "pipefail", "-c"]

# PATH in the base image is broken: https://github.com/ROCm/rocm-jax/issues/124
ENV PATH=/pyenv/shims:/pyenv/bin:/root/bin:/root/.local/bin:/opt/rocm/llvm/bin:/opt/rocm/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
...
RUN --mount=type=cache,target=/root/.cache/pip \
    pip install \
        ...
        # For some reason these are not already installed in the base image https://github.com/ROCm/rocm-jax/issues/123
        'jax==0.6.0' \
        'jaxlib==0.6.0' \
